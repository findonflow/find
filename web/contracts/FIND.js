...
/**
* Method to generate cadence code for FIND transaction
* @param {Object.<string, string>} addressMap - contract name as a key and address where it's deployed as value
*/
export const FINDTemplate = async (addressMap = {}) => {
  ...
  reportMissingImports(CODE, fullMap, `FIND =>`)
...
/**
* Sends FIND transaction to the network
...
export const FIND = async (props) => {
  ...
  const code = await FINDTemplate(addressMap);

  reportMissing("arguments", args.length, 0, `FIND =>`);
  reportMissing("signers", signers.length, 0, `FIND =>`);

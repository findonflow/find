...
func main() {
	...
	flowNfts := map[string][]string{
		...
		// "Wearables" : {"A.1c5033ad60821c97.Wearables.NFT"},
		// "FindPack" : {"A.097bafa4e0b48eef.FindPack.NFT"},
	}
	...
	dapperNFTs := map[string][]string{
		...
		// "float":      {"A.2d4c3caffbeab845.FLOAT.NFT"},
		// "SadBoi" : {"A.d714ab2d9943c4a5.SadboiNFT.NFT"},
	...
	for _, name := range delist {
		o.Tx("removeMarketOption",
			adminSigner,
			WithArg("saleItemName", fmt.Sprintf("Flow%sEscrow", name)),
		)
	}
	...
	for _, name := range delistDapper {
		o.Tx("removeMarketOption",
			adminSigner,
			WithArg("saleItemName", fmt.Sprintf("Dapper%sSoft", name)),
		)
	}
	/*
		o.Tx("adminAddForge",

access(all) contract FindRelatedAccounts {
    ...
    access(all) resource interface Public{
        ...
        // verify enfun sure this wallet address exist under the network
        access(all) fun verify(network: String, address: String) : Bool
        // linked enfun sure this wallet is linked in both wallet with the same name (but not socially linked only)
        // only suppfun orts flow for now
    ...
    access(all) resource Accounts: Public {
        ...
        access(all) fun linked(name: String, network: String, address: Address) : Bool {
            if let cap = FindRelatedAccounts.getCapability(address) {
    ...
    access(all) fun createEmptyAccounts() : @Accounts{
        return <- create Accounts()
    }

    access(all) fun getIdentifier(name: String, network: String, address: String) : String {
    ...
    access(all) fun findRelatedFlowAccounts(address:Address) : {String: [Address]} {
    ...
    access(all) fun findRelatedAccounts(address:Address) : {String: {String: [String]}} {
        let cap = self.getCapability(address)
        if cap == nil {
            return {}
        }

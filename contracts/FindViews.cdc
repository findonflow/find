...
access(all) contract FindViews {
    ...
    access(all) struct AuthNFTPointer : Pointer, AuthPointer{
        ...
        init(cap: Capability<auth(NonFungibleToken.Withdrawable) &{NonFungibleToken.Collection}>, id: UInt64) {
            self.cap=cap
            self.id=id
        ...
        access(all) fun getViewResolver() : &{ViewResolver.Resolver} {
            ...
            let viewResolver = cap.borrowViewResolver(id: self.id) ?? panic("The capability of view pointer is not linked.")
            return viewResolver 
        }

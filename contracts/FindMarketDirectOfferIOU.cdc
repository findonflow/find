...
pub contract FindMarketDirectOfferIOU {
	...
	pub resource SaleItem : FindMarket.SaleItem {
		...
		pub fun acceptEscrowedBid() : @FindIOU.IOU {
			...
			let iou <- self.offerCallback.borrow()!.accept(<- pointer.withdraw(), path:publicPath)
			return <- iou
		}
	...
	pub resource SaleItemCollection: SaleItemCollectionPublic, FindMarket.SaleItemCollectionPublic {
		...
		access(contract) fun registerIncreasedBid(_ id: UInt64) {
			...
			let actionResult=tenant.allowedAction(listingType: Type<@FindMarketDirectOfferIOU.SaleItem>(), nftType: nftType, ftType: ftType, action: FindMarket.MarketAction(listing:true, name: "add bid in direct offer"), seller: self.owner!.address, buyer: saleItem.offerCallback.address)
		...
		access(contract) fun registerBid(item: FindViews.ViewReadPointer, callback: Capability<&MarketBidCollection{MarketBidCollectionPublic}>, validUntil: UFix64?, saleItemExtraField: {String : AnyStruct}) {
			...
			if !self.items.containsKey(id) {
				...
				let actionResult=tenant.allowedAction(listingType: Type<@FindMarketDirectOfferIOU.SaleItem>(), nftType: nftType, ftType: ftType, action: FindMarket.MarketAction(listing:true, name: "bid in direct offer"), seller: self.owner!.address, buyer: callback.address)
		...
		pub fun acceptDirectOffer(pointer: FindViews.AuthNFTPointer, vault: @FungibleToken.Vault?) {
			...
			let actionResult=tenant.allowedAction(listingType: Type<@FindMarketDirectOfferIOU.SaleItem>(), nftType: nftType, ftType: ftType, action: FindMarket.MarketAction(listing:false, name: "fulfill directOffer"), seller: self.owner!.address, buyer: saleItem.offerCallback.address)

			if !actionResult.allowed {
				panic(actionResult.message)
			}

			let cuts= tenant.getTenantCut(name: actionResult.name, listingType: Type<@FindMarketDirectOfferIOU.SaleItem>(), nftType: nftType, ftType: ftType)
			...
			let redeemedVault <- FindIOU.redeem(iou: <- iou, vault: <- vault)

			let resolved : {Address : String} = {}
			resolved[buyer] = buyerName ?? ""
			resolved[owner] = sellerName ?? ""
			resolved[FindMarketDirectOfferIOU.account.address] =  "find" 
	...
	pub resource Bid : FindMarket.Bid {
		...
		access(contract) let iou: @[FindIOU.IOU]
		access(contract) let vaultType: Type
		access(contract) var bidAt: UFix64
		access(contract) let bidExtraField: {String : AnyStruct}

		init(from: Capability<&SaleItemCollection{SaleItemCollectionPublic}>, itemUUID: UInt64, iou: @FindIOU.IOU, nftCap: Capability<&{NonFungibleToken.Receiver}>, bidExtraField: {String : AnyStruct}) {
		...
		access(contract) fun withdrawIOU() : @FindIOU.IOU {
	...
	pub resource interface MarketBidCollectionPublic {
		...
		pub fun containsId(_ id: UInt64): Bool
		access(contract) fun accept(_ nft: @NonFungibleToken.NFT, path:PublicPath) : @FindIOU.IOU
	...
	pub resource MarketBidCollection: MarketBidCollectionPublic, FindMarket.MarketBidCollectionPublic {
		...
		access(contract) let iouReceiver: Capability<&{FindIOU.CollectionPublic}>
		access(contract) let tenantCapability: Capability<&FindMarket.Tenant{FindMarket.TenantPublic}>

		//not sure we can store this here anymore. think it needs to be in every bid
		init(receiver: Capability<&{FungibleToken.Receiver}>, iouReceiver: Capability<&{FindIOU.CollectionPublic}>,tenantCapability: Capability<&FindMarket.Tenant{FindMarket.TenantPublic}>) {
			self.bids <- {}
			self.receiver=receiver
			self.iouReceiver=iouReceiver
			self.tenantCapability=tenantCapability
		}
		...
		pub fun bid(item: FindViews.ViewReadPointer, iou: @FindIOU.IOU, nftCap: Capability<&{NonFungibleToken.Receiver}>, validUntil: UFix64?, saleItemExtraField: {String : AnyStruct}, bidExtraField: {String : AnyStruct}) {
			...
			let bid <- create Bid(from: from, itemUUID:item.getUUID(), iou: <- iou, nftCap: nftCap, bidExtraField: bidExtraField)
	...
	pub fun createEmptyMarketBidCollection(receiver: Capability<&{FungibleToken.Receiver}>, iouReceiver: Capability<&FindIOU.Collection{FindIOU.CollectionPublic}>, tenantCapability: Capability<&FindMarket.Tenant{FindMarket.TenantPublic}>) : @MarketBidCollection {
		return <- create MarketBidCollection(receiver: receiver, iouReceiver: iouReceiver, tenantCapability:tenantCapability)
	}

...
pub contract FindLeaseMarketDirectOfferEscrow {
	...
	pub resource SaleItemCollection: SaleItemCollectionPublic, FindLeaseMarket.SaleItemCollectionPublic {
		...
		access(contract) fun registerIncreasedBid(_ name: String) {
			...
			let actionResult=self.getTenant().allowedAction(listingType: self.getListingType(), nftType: saleItem.getItemType(), ftType: saleItem.getFtType(), action: FindMarket.MarketAction(listing:true, name:"increase bid in direct offer"), seller: self.owner!.address, buyer: saleItem.offerCallback.address)
		...
		access(contract) fun registerBid(name: String, callback: Capability<&MarketBidCollection{MarketBidCollectionPublic}>, validUntil: UFix64?, saleItemExtraField: {String : AnyStruct}) {
			...
			if !self.items.containsKey(name) {
				...
				let actionResult=self.getTenant().allowedAction(listingType: self.getListingType(), nftType: saleItem.getItemType(), ftType: saleItem.getFtType(), action: FindMarket.MarketAction(listing:true, name:"bid in direct offer"), seller: self.owner!.address, buyer: callback.address)
			...
			if saleItem.getBuyer()! == callback.address {
			...
			previousCB.cancelBidFromSaleItem(name)
		...
		pub fun acceptDirectOffer(_ pointer: FindLeaseMarket.AuthLeasePointer) {
			pre {
			...
			let actionResult=tenant.allowedAction(listingType: self.getListingType(), nftType: saleItem.getItemType(), ftType: saleItem.getFtType(), action: FindMarket.MarketAction(listing:false, name:"accept offer in direct offer"), seller: self.owner!.address, buyer: saleItem.offerCallback.address)
	...
	pub resource MarketBidCollection: MarketBidCollectionPublic, FindLeaseMarket.MarketBidCollectionPublic {
		...
		access(contract) fun accept(_ name: String) : @FungibleToken.Vault {
			...
			assert(FindLeaseMarket.getCurrentOwner(name) == self.owner!.address, message: "The lease is not transferred. Offer cannot be fulfilled")

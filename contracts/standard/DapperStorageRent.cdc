...
access(all) contract DapperStorageRent {
    ...
    access(all) fun getStorageRentRefillThreshold(): UInt64 {
    ...
    access(all) fun getRefilledAccounts(): [Address] {
    ...
    access(all) fun getBlockedAccounts() : [Address] {
    ...
    access(all) fun getRefilledAccountInfos(): {Address: RefilledAccountInfo} {
    ...
    access(all) fun getRefillRequiredBlocks(): UInt64 {
    ...
    access(all) fun tryRefill(_ address: Address) {
        /*
    ...
    access(all) fun checkEligibility(_ address: Address): Bool {
        if self.RefilledAccountInfos[address] != nil && getCurrentBlock().height - self.RefilledAccountInfos[address]!.atBlock < self.RefillRequiredBlocks {
            return false
        }
    ...
    access(all) fun cleanExpiredRefilledAccounts(_ batchSize: Int) {
        var index = 0
    ...
    access(all) resource Admin {
        access(all) fun setStorageRentRefillThreshold(_ threshold: UInt64) {
            DapperStorageRent.StorageRentRefillThreshold = threshold
        }

        access(all) fun setRefillRequiredBlocks(_ blocks: UInt64) {
            DapperStorageRent.RefillRequiredBlocks = blocks
        }

        access(all) fun blockAddress(_ address: Address) {
            if !DapperStorageRent.getBlockedAccounts().contains(address) {
        ...
        access(all) fun unblockAddress(_ address: Address) {
            if DapperStorageRent.getBlockedAccounts().contains(address) {

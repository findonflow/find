...
access(all) contract FindUserStatus {
    ...
    access(all) fun getStorefrontListing(user: Address, id: UInt64, type: Type) : StorefrontListing? {

        var listingsV1 : StorefrontListing? = nil
        let account = getAccount(user)
        let storefrontCap = account.capabilities.get<&{NFTStorefront.StorefrontPublic}>(NFTStorefront.StorefrontPublicPath)!
    ...
    access(all) fun getStorefrontV2Listing(user: Address, id: UInt64, type: Type) : StorefrontListing? {
        var listingsV2 : StorefrontListing? = nil
        let account = getAccount(user)
        let storefrontV2Cap = account.capabiltiies.get<&{NFTStorefrontV2.StorefrontPublic}>(NFTStorefrontV2.StorefrontPublicPath)!

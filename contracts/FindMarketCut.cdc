...
access(all) contract FindMarketCut {
    ...
    access(all) fun getRuleId(listingType: Type, nftType:Type, ftType:Type) : String {
        let s : [String] = [listingType.identifier, nftType.identifier, ftType.identifier]
        return FindUtils.joinString(s, sep: "-")
    }

    access(all) fun getCuts(tenant: String, listingType: Type, nftType:Type, ftType:Type): {String : FindMarketCutStruct.Cuts} {
    ...
    access(all) fun getTenantRules(tenant: String, ruleId: String, category: String) : FindMarketCutStruct.Cuts? {
        let contractName = self.categoryToContractName[category] ?? panic("Category is not set to link with contract. Category ".concat(category))
        let con = self.borrowContract(contractName)

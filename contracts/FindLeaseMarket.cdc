...
pub struct AuthLeasePointer {
		...
		init(cap: Capability<&FIND.LeaseCollection{FIND.LeaseCollectionPublic}>, name: String) {
			self.cap=cap
			self.name=name
		...
		access(self) fun borrow() : &FIND.LeaseCollection{FIND.LeaseCollectionPublic} {
		...
		access(account) fun move(to: Address) {
			...
			let profile = receiver.getCapability<&{Profile.Public}>(Profile.ProfilePublicPath)
			return <- self.borrow().move(withdrawID: self.id)
		}

...
func TestFIND(t *testing.T) {
	...
	*/
	...
	ot.Run(t, "Should automatically set Find name to empty if sender have none", func(t *testing.T) {
		...
		).AssertWant(t, autogold.Want("getName pre move 1", "user1"))

		otu.moveNameTo("user1", "user2", "user1")
		otu.O.Script("getName",
			WithArg("address", "user1"),
		).
	...
	ot.Run(t, "Should automatically set Find Name if sender have one", func(t *testing.T) {
		...
		otu.O.Script("getName",
			WithArg("address", "user1"),
		).AssertWant(t, autogold.Want("getName pre move", "user1"))

		otu.moveNameTo("user1", "user2", "user1")

		otu.O.Script("getName",
			WithArg("address", "user1"),
		).AssertWant(t, autogold.Want("getName name1", "name1"))
	...
	/*
		...
		ot.Run(t, "Should be able to getFindStatus of new user", func(t *testing.T) {

			otu.O.Script("getFindStatus",
				WithArg("user", user4Address),
			).AssertWant(t,
		...
		ot.Run(t, "Should be able to resolve find name without .find", func(t *testing.T) {
		...
		//TODO fix this
			...
			ot.Run(t, "Should not be able to list old leases for auction", func(t *testing.T) {

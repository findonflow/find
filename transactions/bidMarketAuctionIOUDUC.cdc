...
transaction(marketplace:Address, user: String, id: UInt64, amount: UFix64) {
	...
	prepare(dapper: AuthAccount, account: AuthAccount) {
	 ...
	 /// auctions that refers FT so 'IOU' auction
		...
		self.saleItemsCap= FindMarketAuctionIOU.getSaleItemCapability(marketplace:marketplace, user:address) ?? panic("cannot find sale item cap")
		...
		let bidStoragePath=tenant.getStoragePath(Type<@FindMarketAuctionIOU.MarketBidCollection>())

		self.bidsReference= account.borrow<&FindMarketAuctionIOU.MarketBidCollection>(from: bidStoragePath)
		...
		self.iou <- FindIOU.create(<- self.mainDapperUtilityCoinVault.withdraw(amount: amount))
	...
	execute {
		self.bidsReference!.bid(item:self.pointer, iou: <- self.iou, nftCap: self.targetCapability, bidExtraField: {})
	}

...
transaction(merchAccount: Address, name: String, amount: UFix64) {
    ...
    prepare(dapper: auth(BorrowValue) &Account, account: auth(BorrowValue) &Account) {
        ...
        self.finLeases= account.storage.borrow<auth(FIND.LeaseOwner) &FIND.LeaseCollection>(from:FIND.LeaseStoragePath) ?? panic("Could not borrow reference to find lease collection")
    }

...
transaction(lease: String, typeId: UInt64, thumbnailHash: String, wallet: Address, openTime:UFix64, royaltyCut: UFix64, royaltyAddress: Address, startTime:{Int : UFix64}, endTime: {Int : UFix64}, floatEventId: {Int : UInt64}, price: {Int : UFix64}, purchaseLimit:{Int: UInt64}) {
	...
	execute {
		...
		let providerCap= self.admin.getProviderCap(ExampleNFT.CollectionPrivatePath)
		...
		for key in startTime.keys {
			...
			var verifier : {FindVerifier.Verifier}? = nil 
			if floatEventId[key] != nil {
				verifier = FindVerifier.HasOneFLOAT([floatEventId[key]!])
			}

			saleInfo.append(FindPack.SaleInfo(startTime : startTime[key]! , endTime : endTime[key] , price : price, purchaseLimit: purchaseLimit[key], verifier: verifier))
		}

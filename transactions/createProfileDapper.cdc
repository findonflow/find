...
transaction(name: String) {
    prepare(account: auth(Profile.Owner, StorageCapabilities, SaveValue,PublishCapability, BorrowValue) &Account) {
        ...
        if !profile.hasWallet("DUC") {
            ...
            profile.addWallet(Profile.Wallet( name:"DUC", receiver:ducReceiver, balance:account.capabilities.get<&{FungibleToken.Valut}>(/public/dapperUtilityCoinVault)!, accept: Type<@DapperUtilityCoin.Vault>(), tags: ["duc", "dapperUtilityCoin","dapper"]))
            updated=true
        }

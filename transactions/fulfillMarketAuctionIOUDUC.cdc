...
transaction(marketplace:Address, id: UInt64, amount:UFix64) {
	...
	prepare(dapper: AuthAccount, account: AuthAccount) {
		...
		self.bidsReference= account.borrow<&FindMarketAuctionIOU.MarketBidCollection>(from: storagePath) ?? panic("This account does not have a bid collection")

		let marketOption = FindMarket.getMarketOptionFromType(Type<@FindMarketAuctionIOU.MarketBidCollection>())

...
transaction(dapperAccount: Address, marketplace:Address, nftAliasOrIdentifier:String, id: UInt64, price:UFix64, auctionReservePrice: UFix64, auctionDuration: UFix64, auctionExtensionOnLateBid: UFix64, minimumBidIncrement: UFix64, auctionValidUntil: UFix64?) {
	...
	prepare(account: AuthAccount) {
		...
		if !ducReceiver.check() {
			...
			let dapper = getAccount(dapperAccount)
		...
		if !profile.hasWallet("DUC") {
			profile.addWallet(Profile.Wallet( name:"DUC", receiver:ducReceiver, balance: getAccount(dapperAccount).getCapability<&{FungibleToken.Balance}>(/public/dapperUtilityCoinBalance), accept: Type<@DapperUtilityCoin.Vault>(), tags: ["duc", "dapperUtilityCoin","dapper"]))
			updated=true
		}

...
transaction(id: UInt64, directSellPrice:UFix64) {
	prepare(account: AuthAccount) {
		...
		let saleItems= account.borrow<&FindMarketSale.SaleItemCollection>(from: tenant.getStoragePath(Type<FindMarketSale.SaleItemCollection>()) ?? panic("Cannot find saleItems")
